  ...
  QDialog (pa, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  shortName = QFileInfo (misc->progname).fileName ();
      ...
      if (wktString.contains ("PROJCS"))
        {
          ...
          if (!(misc->pj_utm = pj_init_plus (string)))
            {
              QMessageBox::critical (parent, "fileEdit3D", tr ("%1 %2 %3 %4 - Error initializing UTM projection!").arg
                                     (misc->progname).arg (__FILE__).arg (__FUNCTION__).arg (__LINE__));
              exit (-1);
            }
          ...
          if (!(misc->pj_latlon = pj_init_plus (string)))
            {
              QMessageBox::critical (parent, "fileEdit3D", tr ("%1 %2 %3 %4 - Error initializing latlon projection!").arg
                                     (misc->progname).arg (__FILE__).arg (__FUNCTION__).arg (__LINE__));
              exit (-1);
            }
        }
  ...
  if (!text.isEmpty ())
    {
      if ((!text.startsWith ("GEOGCS")) && (!text.startsWith ("PROJCS")))
        {
          QMessageBox::warning (parent, tr ("%1 Well-known Text").arg (shortName),
