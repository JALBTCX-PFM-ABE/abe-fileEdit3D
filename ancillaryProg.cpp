...
ancillaryProg::ancillaryProg (QWidget *parent, nvMapGL *ma, POINT_DATA *da, OPTIONS *op, MISC *mi, int32_t prog, 
                              int32_t lock_point):
  QProcess (parent)
{
  int32_t buildCommand (QString progString, QString actionString, POINT_DATA *data, MISC *misc, int32_t nearest_point, QString *cmd,
                        QStringList *args, int32_t kill_switch);
  ...
  int32_t ret = buildCommand (progString, actionString, data, misc, lock_point, &cmd, &args, options->kill_switch[prog]);
      ...
      if (appTimerCount > 10)
        {
          ...
          QMessageBox::information (pa, tr ("fileEdit3D ancillary program"), tr ("The program %1 has been started but may take some time to appear.\n"
                                                                                 "Please be patient, it will appear eventually.").arg (program));
          qApp->processEvents ();
        }
...
ancillaryProg::slotError (QProcess::ProcessError error)
{
  ...
  switch (error)
    ...
    case QProcess::FailedToStart:
      QMessageBox::critical (pa, tr ("fileEdit3D ancillary program"), tr ("Unable to start the ancillary program!\n%1").arg (commandLine));
      break;

    case QProcess::Crashed:
      QMessageBox::critical (pa, tr ("fileEdit3D ancillary program"), tr ("The ancillary program crashed!\n%1").arg (commandLine));
      break;

    case QProcess::Timedout:
      QMessageBox::critical (pa, tr ("fileEdit3D ancillary program"), tr ("The ancillary program timed out!\n%1").arg (commandLine));
      break;

    case QProcess::WriteError:
      QMessageBox::critical (pa, tr ("fileEdit3D ancillary program"), tr ("There was a write error to the ancillary program!\n%1").arg (commandLine));
      break;

    case QProcess::ReadError:
      QMessageBox::critical (pa, tr ("fileEdit3D ancillary program"), tr ("There was a read error from the ancillary program!\n%1").arg (commandLine));
      break;

    case QProcess::UnknownError:
      QMessageBox::critical (pa, tr ("fileEdit3D ancillary program"), tr ("The ancillary program died with an unknown error!\n%1").arg (commandLine));
      break;
