...
{
  double xyz_y = ((y - data->bounds.min_y) / edit_y_range) * misc->draw_area_height;
  double xyz_x = ((x - data->bounds.min_x) / edit_x_range) * misc->draw_area_width;
  ...
  for (int32_t i = 0 ; i < data->count ; i++)
    {
      if (!check_bounds (options, misc, data, i, NVTrue, misc->slice))
        {
          int32_t z = pseudo_dist_from_viewer (misc, data, data->x[i], data->y[i]);
