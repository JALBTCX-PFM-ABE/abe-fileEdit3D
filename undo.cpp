          ...
          for (int32_t i = 0 ; i < undo_levels - 1 ; i++)
            {
              ...
              try
                ...
                {
                  QMessageBox::critical (0, fileEdit3D::tr ("fileEdit3D store undo"),
                                         fileEdit3D::tr ("Unable to allocate UNDO validity memory!  Reason : %1").arg (strerror (errno)));
                  exit (-1);
                }
              ...
              try
                ...
                {
                  QMessageBox::critical (0, fileEdit3D::tr ("fileEdit3D store undo"),
                                         fileEdit3D::tr ("Unable to allocate UNDO num memory!  Reason : %1").arg (strerror (errno)));
                  exit (-1);
                }
      ...
      if (misc->undo[ucnt].count)
        {
          ...
          for (int32_t i = 0 ; i < misc->undo[ucnt].count ; i++) data->val[misc->undo[ucnt].num[i]] = misc->undo[ucnt].val[i];


          //  Clear the undo memory and reset the count.
...
uint8_t resize_undo (MISC *misc, OPTIONS *options, int32_t undo_levels)
{
  ...
  if (undo_levels <= misc->undo_count)
    {
      QMessageBox::warning (0, fileEdit3D::tr ("fileEdit3D resize undo"),
                            fileEdit3D::tr ("Resizing to less than already existing undo blocks is not allowed!"));
      return (NVFalse);
    }
